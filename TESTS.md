...
nothing. Measured on AGX Orin (JetPack 5, Node 20.20.2): 3.32 Mbps at 0% loss for 34-39% of one
core across repeated idle runs. Both figures are taken over the same media window; an earlier
"roughly a quarter" divided the CPU by a longer one and understated the cost.
...
| `tests/video/` | VID-SDP-01, VID-LIFE-01/02, VID-LEAK-01 through a real ROS graph and a real decoder; `VIDEO_BACKEND` also runs VID-PROBE-01 and VID-HW-01 against a real encoder | VID-AUTH-01, which no scenario here exercises: authorization is covered by unit tests only |
...
The bridge offers host candidates, plus a server-reflexive candidate when a deployment sets
`BRIDGE_ICE_STUN_URL`. It never configures a TURN server, so a relayed video path remains the
browser's `RTCConfiguration` rather than anything this side negotiates, and no `VID-*` identifier
covers one.

...
CONTRACT_VERSION = 1
MAX_LINE_BYTES = 8192
# Frames pushed through the real pipeline to prove it encodes; the geometry comes from the track.
PROBE_FRAMES = 10
...
class Encoder:
    ...
    def push(self, message):
        ...
        # push displaces the frame in it.
        if self.source.get_property('current-level-buffers') >= self.source.get_property('max-buffers'):

...
export function createWorkerFactory(port: WorkerPort): MediaSourceFactory {
  return (binding: VideoBinding): MediaSource => {
    ...
    // Set the moment a stop is requested, and consulted by every failure path. Without it the normal
    // shutdown of the last viewer's worker reports a failure after the fact, which drives the source
    // to `failed` and - if a new viewer already started a replacement - stops that replacement.
    let stopping = false;
    ...
    const begin = (request: Record<string, unknown>, onPacket?: (packet: Buffer) => void,
      ...
      const fail = (error: Error): void => {
        // Everything a worker does on its way out is shutdown noise, not a source failure. Stopping
        // kills it wherever it happens to be: mid RFC 4571 frame, mid NDJSON line. Those bytes reach
        // the deframers and look exactly like a contract violation, and treating one as a failure
        // leaves the track marked failed long after it was deliberately released.
        if (stopping) return;

 ...
 * @param value Raw map, e.g. `{start_timeout_ms: 5000, stop_grace_ms: 5000, pli_min_interval_ms: 200}`.
...
function videoSettings(value: unknown): VideoSettings {
  const map = record(value, ['start_timeout_ms', 'stop_grace_ms', 'pli_min_interval_ms'], 'video');

...
Deployments may configure one validated STUN URI and a fixed UDP allocation range for server-reflexive ICE candidates. The URI parser accepts only the supported `stun:<host>[:port]` form: an ASCII DNS name, strict IPv4 address, or bracketed IPv6 address with an optional decimal port whose value is from 1 through 65535. It performs syntax validation without DNS resolution and rejects `stuns:`, URI authority syntax, credentials, paths, queries, fragments, and invalid ports before native modules load. These settings make firewall rules deterministic but do not by themselves guarantee Internet reachability; direct public paths and TURN fallback remain deployment-specific acceptance tests.

...
Deployments may configure one validated STUN URI and a fixed UDP allocation range for server-reflexive ICE candidates. These settings make firewall rules deterministic but do not by themselves guarantee Internet reachability; direct public paths and TURN fallback remain deployment-specific acceptance tests.

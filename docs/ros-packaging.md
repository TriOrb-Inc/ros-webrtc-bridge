...
Concurrent initializers publish a fully written private inode with an atomic, non-replacing hardlink on the same filesystem, then read the winner's value. Before `ensure` returns, it fsyncs the containing directory and every ancestor from child to root, persisting the credential link and newly created directory entries. This also applies when reusing an existing value, since another initializer may still be completing its own directory sync. Directory-open/fsync failures reject with the same fixed error; a published value may remain and is reused on retry. The filesystem must support directory fsync as well as hardlinks. Candidate directories are removed even after a failed publication; a process killed mid-operation can leave a private `.credential-*` staging directory for administrator cleanup. The target is never overwritten. Use a filesystem supporting hardlinks.

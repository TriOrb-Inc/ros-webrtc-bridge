...
async function exchange() {
  ...
  return {
    ...
    // Measured over the same window as the throughput above: `elapsedMs` also covers the drain that
    // follows the media, and dividing by it made the published cost depend on how long that is.
    cpu_pct_of_one_core: Number(((cpuMs / (mediaSeconds * 1000)) * 100).toFixed(1)),
...
try {
  ...
  // Without this the `finally` below exits zero while an assertion is still in flight, so every
  // check in this probe is unenforced and a mandatory pre-PR step can never fail.
  console.error(error instanceof Error ? error.message : 'media smoke failed');
  process.exitCode = 1;
} finally {

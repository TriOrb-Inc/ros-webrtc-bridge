...
export async function browserVideoLoadScenario(input: VideoLoadInput): Promise<VideoLoadResult> {
  ...
  async function watcher() {
    ...
    return {
      ...
      subscribe: () => control.send(JSON.stringify({ v: 1, op: 'video.subscribe', id: `s${Math.random()}`, track })),
  ...
  try {
    ...
    return {
      ...
      assertions: { everyViewerDecoded: 'PASS', departureDidNotDisturbOthers: 'PASS' },
    };
  } catch (error) {

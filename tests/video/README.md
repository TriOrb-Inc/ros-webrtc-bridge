...
| Browser | Container | A decoder is what is being tested, so it must be a real one. Chromium runs in a third container on the same private network, so it reaches the gateway directly; this process only drives it over CDP, and the host needs no Playwright-supported platform |

All three containers join a `--internal` Docker network with a test-specific `ROS_DOMAIN_ID`, so DDS

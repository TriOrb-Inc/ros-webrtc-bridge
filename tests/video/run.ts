...
async function verify(distro: 'humble' | 'jazzy', backend: Backend, mode: 'verify' | 'load') {
  ...
  try {
    ...
    const rosArgs = ['--network', network, '--label', `ros-webrtc-test=${suffix}`, '--env', 'ROS_DOMAIN_ID=74',
      // The publisher is the only image source, and its own clock starts before the image build's
      // successors: readiness, discovery, the browser server and then the scenario itself, which in
      // load mode alone is budgeted at ten minutes. At 360 s it disappeared mid-run and nothing
      // noticed, because the peer is never re-checked after the one-time topic poll.
      '--env', 'ROS_TEST_TIMEOUT_SECONDS=2400', '--env', 'RMW_IMPLEMENTATION=rmw_fastrtps_cpp',
    ...
    /**
     * Fail unless the only image source outlived the run.
     * @returns Completion. A scenario that passed while the publisher was gone proved something else,
     * and nothing re-checks the peer after the one-time topic poll.
     */
    const publisherSurvived = async (): Promise<void> => {
      const alive = await run('peer-alive', 'docker', ['inspect', '--format', '{{.State.Running}}', peer]);
      assert.equal(alive.trim(), 'true', 'the image publisher stopped before the run finished');
    };

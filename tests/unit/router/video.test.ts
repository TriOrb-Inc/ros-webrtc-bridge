...
test('detaches exactly what the media plane is still holding', () => {
  // A failed source lets go of its viewers itself, so closing afterwards has nothing to take out.
  // Detaching anyway would remove a viewer from whatever source had replaced it.
  const failed = fixture();
  call(failed, 'video.subscribe', { id: 'r1', track: 'front' });
  failed.viewers.get('front')!.state('failed');
  failed.router.close();
  assert.deepEqual(failed.detached, []);

  // A watched track is still held, so closing releases it exactly once.
  const watching = fixture();
  call(watching, 'video.subscribe', { id: 'r1', track: 'front' });
  watching.router.close();
  assert.deepEqual(watching.detached, ['front']);

  // And a peer that was refused an attachment has nothing to take out either.
  const refused = fixture([]);
  assert.throws(() => call(refused, 'video.subscribe', { id: 'r1', track: 'front' }));
  refused.router.close();
  assert.deepEqual(refused.detached, []);
});

test('keeps attach and detach paired through failure, retry and close', () => {
  // `watching` and `attached` were split apart to fix a leak, so the sequences that cross them are
  // where a new one would hide. Every attach must be matched by exactly one detach.
  const f = fixture();
  call(f, 'video.subscribe', { id: 'r1', track: 'front' });
  f.viewers.get('front')!.state('failed');
  call(f, 'video.subscribe', { id: 'r2', track: 'front' });     // the documented retry
  assert.deepEqual(f.attached, ['front', 'front']);
  assert.deepEqual(f.detached, [], 'a retry does not detach');
  f.router.close();
  assert.deepEqual(f.detached, ['front'], 'and the peer leaves exactly once');

  // Unsubscribing after a failure must also leave exactly once, and allow a fresh subscription.
  const g = fixture();
  call(g, 'video.subscribe', { id: 'r1', track: 'front' });
  g.viewers.get('front')!.state('failed');
  const mid = call(g, 'video.subscribe', { id: 'r2', track: 'front' }).mid as string;
  call(g, 'video.unsubscribe', { id: 'r3', mid });
  assert.deepEqual(g.detached, ['front']);
  call(g, 'video.subscribe', { id: 'r4', track: 'front' });
  assert.deepEqual(g.attached, ['front', 'front', 'front']);
  g.router.close();
  assert.deepEqual(g.detached, ['front', 'front'], 'two attachments released, not three');
});

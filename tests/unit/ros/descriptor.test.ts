...
test('TYPE-01 ROS descriptor: generate primitive, nested, and array descriptors from explicit metadata', () => {
  ...
  // Compare primitive values against independent fixed descriptors.
...
test('CFG-01 ROS descriptor: reject unsupported types, cycles, depth, and invalid metadata', () => {
  ...
  // Reproduce nested recursion cycles separately from simple depth overflow.
...
test('TYPE-01 ROS representation: independently validate ref-napi integers/bytes and bridge native values', () => {
  ...
  // Publish bigint values and ordinary uint8 arrays; preserve ordinary strings unchanged.
...
test('SEC-01 ROS representation: reject invalid tree shapes and allocation limits', () => {
  ...
  // Reject getters, holes, and extra properties rather than discarding them.

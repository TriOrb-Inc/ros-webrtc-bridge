...
const SETTINGS = { start_timeout_ms: 5000, stop_grace_ms: 5000, pli_min_interval_ms: 200 };
const TRACK = {
...
test('accepts a complete media plane and freezes it', () => {
  ...
  assert.deepEqual(video.settings, { startTimeoutMs: 5000, stopGraceMs: 5000, pliMinIntervalMs: 200 });
...
test('validates every shared timing', () => {
  for (const key of ['start_timeout_ms', 'stop_grace_ms', 'pli_min_interval_ms']) {

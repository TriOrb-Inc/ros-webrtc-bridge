...
function encoder(behaviour: { startThrows?: boolean; stopThrows?: boolean; keyframeThrows?: boolean } = {}) {
  ...
  return { factory, state,
    ...
    emit: (value = 1, sequence = value, timestamp = value * 3000) => emit?.(rtpPacket(value, sequence, timestamp)),
...
test('starts on the first viewer and shares one encoder with the rest', () => {
  ...
  h.encoder.emit(2);
  assert.deepEqual(first.packets.map(p => p[12]), [1, 2]);
  assert.deepEqual(second.packets.map(p => p[12]), [2]);
...
test('resumes inside the grace window without restarting the encoder', () => {
  ...
  assert.deepEqual(second.states, ['active']);
  h.encoder.emit(7);
  assert.deepEqual(second.packets.map(p => p[12]), [7]);
});
...
test('does not start a replacement while the previous encoder is still going away', async () => {
  ...
  await settle();
  packet?.(rtpPacket());            // reaching `active` cancels the start deadline
...
test('starts nothing after close, even mid-release', async () => {
  // Shutdown can arrive while a restart is waiting for the previous encoder to go away. Its own
  // finish sees no source to stop, because the replacement has not been created yet.
  let release: (() => void) | undefined;
  let packet: ((value: Buffer) => void) | undefined;
  ...
  packet!(rtpPacket());         // reaching `active` cancels the start deadline
  service.detach('front', first.sink);
  time.advance(5000);                // only the grace window fires, so this is not a failure
  await settle();
  service.attach('front', second.sink);
  await settle();
  ...
  await closing;
  await settle();
  assert.equal(state.started, 1, 'the suspended start must not outlive the service');
});
